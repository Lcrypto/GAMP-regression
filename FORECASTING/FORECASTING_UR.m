...
%==========================================================================
% This code replicates the forecasting results in Tables 1 & 2 of Korobilis (2019)
% "High-dimensional macroeconomic forecasting using message passing
% algorithms"
% 
% Instructions: 
% - Make sure you choose either CPI when running this code (set "index_variable = 95" in USER INPUT), or 
%   PCE inflation (set "index_variable = 105" in USER INPUT).
...
%   around line 150) and all subsequent lines of code that use output from these functions (e.g. y_hat_TVPTVD and 
...
index_variable = 105;    % 95:CPI; 105:PCE;
...
Yraw_full = (1200/nfore)*yfcsta(transxf(data(:,index_variable),5),5,nfore);   %Yraw_full = adjout(Yraw_full,4.5,4);
Yraw_full_lags = 1200*transxf(data(:,index_variable),5);                      %Yraw_full_lags = adjout(Yraw_full_lags,4.5,4);
DYraw_full_lags = diff(Yraw_full_lags);
Xraw = 0*data;
...
Xraw = Xraw(3:end,:);  Yraw_full = Yraw_full(3:end,:); Yraw_full_lags = Yraw_full_lags(3:end,:); DYraw_full_lags = DYraw_full_lags(2:end,:);

% Now correct sizes for h-step ahead forecasts
Yraw_full = Yraw_full(1:end-nfore,:);  
Yraw_full_lags = Yraw_full_lags(1:end-nfore,:);
DYraw_full_lags = DYraw_full_lags(1:end-nfore,:);
...
for sample = T_thres:T_full
    ...
    ylag = mlag2(DYraw_full_lags,lags-1);  % This routine creates the lagged dep variables
%     F = extract(zscore(Xraw(1:T+nfore,find(fcode==1))),p);
    ...
    if constant == 1       
        Zraw  = [ones(size(Yraw_full_lags(lags:sample+nfore,:),1),1) ylag(lags:sample+nfore,:) Xraw(lags:sample+nfore,:)];
        Zraw2 = [ones(size(Yraw_full_lags(lags:sample+nfore,:),1),1) ylag(lags:sample+nfore,:) FF(lags:sample+nfore,:)];
    elseif constant == 0                        
        Zraw  = [Yraw_full_lags(lags:sample+nfore,:) ylag(lags:sample+nfore,:) Xraw(lags:sample+nfore,:)];
        Zraw2 = [Yraw_full_lags(lags:sample+nfore,:) ylag(lags:sample+nfore,:) FF(lags:sample+nfore,:)];
    end
    % Correct # of observations of y after taking lags
    [Yraw1,mm,ss] = zscore(Yraw_full(lags:sample,:));
    [Yraw2,~,~] = zscore(Yraw_full_lags(lags:sample+nfore,:));
    Yraw = Yraw1 - Yraw2(1:end-nfore,:);
...
%     columnNorms = sqrt(diag(Zraw'*Zraw)); Zraw = Zraw*diag(1 ./ columnNorms);
    ...
    Yraw_fore = Yraw_full(sample+nfore,:);% - Yraw_full_lags(sample+nfore,:);
    ...
    [beta_SSVS,~,sigma_SSVS,~]       = SSVS(X2,y,ndraws,nburn);

    % b\ tvp regressions with predictors
    ...
    %% Forecasting
    if forecasting == 1
        y_hat_ARp     = mm + ss.*((X_fore(:,1:q)*beta_ARp + sqrt(sigma_ARp)*randn(1,ndraws)) + Yraw2(end,:));
        y_hat_KP      = mm + ss.*(y_f_KP' + Yraw2(end,:));
        y_hat_GK      = mm + ss.*(y_f_GK' + Yraw2(end,:));
        y_hat_TVPAR   = mm + ss.*((X_fore(:,1:q)*squeeze(beta_TVPAR(end,:,:)) + sqrt(sigma_TVPAR(end,:)).*randn(1,ndraws)) + Yraw2(end,:));
        y_hat_UCSV    = mm + ss.*((beta_UCSV(end,:) + sqrt(sigma_UCSV(end,:)).*randn(1,ndraws)) + Yraw2(end,:));
        y_hat_TVPTVD  = mm + ss.*(y_hat_TVPTVD' + Yraw2(end,:));
        y_hat_TVPTVS  = mm + ss.*((X_fore2(:,1:q+3)*squeeze(beta_TVPTVS(end,:,:)) + sqrt(sigma_TVPTVS(end,:)).*randn(1,ndraws/2)) + Yraw2(end,:));
        y_hat_SSVS    = mm + ss.*((X_fore2*beta_SSVS + sqrt(sigma_SSVS)*randn(1,ndraws)) + Yraw2(end,:));
        y_hat_TVPBMA  = mm + ss.*((X_fore2(:,1:13)*squeeze(beta_TVPBMA(end,:,:)) + sqrt(sigma_TVPBMA(end,:)).*randn(1,ndraws)) + Yraw2(end,:));
        y_hat_SBLTV   = mm + ss.*(([X_fore2, XX_fore]*beta_SBLTV + sqrt(sigma_SBLTV(end))*randn(1,ndraws)) + Yraw2(end,:));
        ...
        med_f_ARp     = median(y_hat_ARp',1);       
        med_f_KP      = median(y_hat_KP',1);
        med_f_GK      = median(y_hat_GK',1);
        med_f_TVPAR   = median(y_hat_TVPAR',1);
        med_f_UCSV    = median(y_hat_UCSV',1);
        med_f_TVPTVD  = median(y_hat_TVPTVD',1);
        med_f_TVPTVS  = median(y_hat_TVPTVS',1);
        med_f_SSVS    = median(y_hat_SSVS',1);
        med_f_TVPBMA  = median(y_hat_TVPBMA',1);
...
save(sprintf('%s_%s_%g_%s_%g.mat',cell2mat(Ynames(index_variable)),'UR_(_h_=',nfore,')',rand),'-mat');