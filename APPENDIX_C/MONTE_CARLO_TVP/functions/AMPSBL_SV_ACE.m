function [beta_means,beta_vars,sigma2,lambdas] = AMPSBL_SV_ACE(X,y,maxiter)

% Define prior moments of alpha ~ Gamma(a,b)
a = 1e-20;%1-0.2*size(X,1)/size(X,2);
b = 1e-20;
...
%     clc
    ...
    for jj=1:1
        ...
        resid  = y-z;
        rho    = (resid(1:end-1)'*resid(1:end-1))\(resid(1:end-1)'*resid(2:end));
        y_til  = y - resid*rho;         
        
        % Set sigma.sq equal to rough estimate of posterior mode
        ly2 = log((y_til-z).^2 + 1e-10);
        ...
        z_hat  =  tau_z.*(y_til./sigma2 + p_hat./tau_p);      % E(z(i) | y,p_hat(k,i),tau_p(k,i))
