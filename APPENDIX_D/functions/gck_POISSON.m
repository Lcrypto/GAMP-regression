function [kdraw,lpy2n] = gck_POISSON(yg,hh,capg,capf,sigv,kold,t,ex0,vx0,nvalk,lambda,kvals,p,kstate)
...
for i = 2:t
    ...
    for j = 1:nvalk
        ...
        ggg = gat*gat';
        fvf = ft*vtm1*ft' + ggg;
        rt = ht'*fvf*ht + capg((i-1)*p+1:i*p,:)*capg((i-1)*p+1:i*p,:)';
        rtinv = inv(rt);
        jt = fvf*ht*rtinv;
        ...
        % Draw from the Poisson prior
        % First find durations d(1t), d(2t) and d(3t) as in Section 5 of Giordani and Kohn
        tau1 = find(kdraw(1:i-1,:)==1, 1, 'last' );
        tau2 = find(kold(i+1:t,:)==1, 1 ) + i;
        if isempty(tau1); tau1=0; end;
        if isempty(tau2); tau2=0; end;
        d1t = abs(i - tau1);
        d2t = abs(tau2 - i);
        d3t = abs(tau2 - tau1);
        if i<t
            prob_K(1,1) = poisspdf(d1t-1,lambda)*poisspdf(d2t-1,lambda); %p(Kt=1 | K)
            prob_K(2,1) = poisspdf(d3t-1,lambda); %p(Kt=0 | K)
        end
        
        lprob(j,1) = log(prob_K(j,1)+1e-9) + lpyt1n + lpyt;
        if i == 2;
            lpy2n = lpyt1n + lpyt;
        end
    end
