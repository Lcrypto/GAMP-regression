function [Bt_draws,Sigt_draws] = TVP_BMA(y,x,plag,nrep,nburn)
...
fprintf('Now you are running TVP_BMA')
fprintf('\n')
...
for irep = 1:nrep + nburn    % GIBBS iterations starts here
    ...
    [Btdraw,~] = carter_kohn_bma(y,Z,sigtS,Qdraw,m,p,t,B_0_prmean,B_0_prvar,gammas);
...
%         else
        if j > plag+1
            ...
            x_star_star = 0;
            for ii = 1:t           
                xtemp1 = (y(:,ii) - Z((ii-1)*p+1:ii*p,:)*theta_j_star(:,ii));
                xtemp2 = (y(:,ii) - Z((ii-1)*p+1:ii*p,:)*theta_j_star_star(:,ii));
                x_star = x_star + (xtemp1'*xtemp1)./sigtS(ii);
                x_star_star = x_star_star + (xtemp2'*xtemp2)./sigtS(ii); 
            end
            c_j = p_j(j)*exp(-1/2*x_star);
            d_j = (1-p_j(j))*exp(-1/2*x_star_star);
            p_j_tilde = c_j / (c_j+d_j);
            gammas(j,1) = bernoullirnd(p_j_tilde);    
        end        
    end
    ...
    y2 = [];
    for ii = 1:t
        ytemps = y(:,ii) - Z((ii-1)*p+1:ii*p,:)*diag(gammas)*Btdraw(ii,:)';
